  ...
  return (
    <main className="min-h-screen bg-cream pt-24">
      ...
      <section className="relative h-[50vh] min-h-[380px] w-full overflow-hidden">
        ...
        <div className="relative z-10 mx-auto flex h-full max-w-4xl flex-col items-center justify-center px-4 text-center sm:px-6">
          <motion.div
            ...
            <LocalizedHeading className="font-display mt-4 text-4xl font-semibold text-white sm:text-6xl">{t('title')}</LocalizedHeading>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        <h2 className="mb-6 font-display text-2xl font-semibold text-forest">{copy('Places to explore')}</h2>
        <FilterBar
          search={search}
          onSearchChange={setSearch}
          searchPlaceholder={t('searchPlaceholder')}
          filters={[{ key: 'category', label: t('filters.categories'), options: CATEGORY_OPTIONS.map(option => ({ ...option, label: copy(option.label) })) }]}
          values={{ category }}
